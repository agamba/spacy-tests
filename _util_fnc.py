...
def get_noun_adj(noun_chunk_str, keep_det=True):
    '''
    formats noun + adj.
    ...
    # add det
    if (keep_det):
        noun_adj = "[" + determiner + " " + noun + " ] " + adjectives
    else:
        noun_adj = "[" + noun + " ] " + adjectives
...
def analyze_sentence(sentence):
    ...
    parts = {'tense': '', 'verb': '', 'lema': '', 'subject': '', 'object': ''}
    ...
    for w in sentence:
        ...
        # check for the tense of the root verb
        if (w.dep_ == "ROOT"):
            # convert morphology string to a dict
            verb_morph = Morphology.feats_to_dict(str(w.morph))
            # print(verb_morph)
            try:
                # check for present and past progressive: verb to be before root verb
                if (sentence[counter_tok-1].dep_ == "aux" and (sentence[counter_tok-1].text == "am" or sentence[counter_tok-1].text == "is" or sentence[counter_tok-1].text == "are" or sentence[counter_tok-1].text == "was" or sentence[counter_tok-1].text == "were")):
                    # print("---> TO BE + " + w.text)
